...
def generate_image(favicon_link, thumbnail_link, newssource, headline, date, mode):
    ...
    # int(thumbnail_width*0.75)
    thumbnail_height = int(
        (original_thumbnail_height/original_thumbnail_width)*thumbnail_width)
    ...
    draw_thumb.rounded_rectangle(
        (0, 0, thumbnail_width, thumbnail_height), radius=15, fill=255)
    ...
    newssource_width, newssource_height = newssource_font.getsize(
        new_newssource)
    ...
    if mode == 'dark':
        draw.text((headline_x, headline_y), new_headline.strip(),
                  fill=(255, 255, 255), font=headline_font)
    else:
        draw.text((headline_x, headline_y), new_headline.strip(),
                  fill=(0, 0, 0), font=headline_font)
    ...
    if mode == 'dark':
        draw.text((newssource_x, newssource_y), new_newssource,
                  fill=(255, 255, 255), font=newssource_font)
    else:
        draw.text((newssource_x, newssource_y), new_newssource,
                  fill=(0, 0, 0), font=newssource_font)

    # Draw the date
    date_font = ImageFont.truetype("arial.ttf", 12)
    if mode == 'dark':
        draw.text((newssource_x+newssource_width, newssource_y),
                  date, fill=(255, 255, 255), font=date_font)
    else:
        draw.text((newssource_x+newssource_width, newssource_y),
                  date, fill=(0, 0, 0), font=date_font)
    ...
    draw_img.rounded_rectangle(
        (0, 0, image_width, image_height), radius=15, fill=255)

    # Apply the circular mask to the image
    bg_image.putalpha(mask_img)

    bg_image = bg_image.resize(
        (1500, int(1500*bg_image.size[1]/bg_image.size[0])))
    bg_image = bg_image.filter(ImageFilter.UnsharpMask(
    ...
    if mode == 'dark':
        final_image = Image.new(
            'RGB', (final_image_width, final_image_height), (30, 30, 30))
    else:
        final_image = Image.new(
            'RGB', (final_image_width, final_image_height), (195, 195, 195))
    final_image.paste(bg_image, ((
        final_image_width-bg_image.size[0])//2, (final_image_height-bg_image.size[1])//2), bg_image)
    return final_image
...
def get_image(favicon, img, website, title, date, mode):
    # Get the screenshot
    ...
    image = generate_image(favicon, img, website, title,
                           date, mode)  # pass parameter
